...
public class TestTWStock
{
	...
	public void testGetStockInfo() {
		...
		try {
			...
			request = new HttpGet( "http://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=tse_2330.tw&_=" + Calendar.getInstance().getTimeInMillis() );
			response = client.execute( request );
			...
			TWStockPriceInfo stockPriceInfo = new ObjectMapper().readValue( StringUtils.trim( IOUtils.toString( response.getEntity().getContent(), "UTF-8" ) ),
			        TWStockPriceInfo.class );

			for (TWStockPrice price : stockPriceInfo.getStockPrices()) {
				logger.info( price.getHighestPrice() );
			}
