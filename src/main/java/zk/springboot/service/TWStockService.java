...
public class TWStockService
{
	...
	public TWStockPrice getTodayStockPrice(String stock) {
		...
		try {
			...
			TWStockPriceInfo stockPriceInfo = objectMapper.readValue( StringUtils.trim( IOUtils.toString( response.getEntity().getContent(), "UTF-8" ) ),
			        TWStockPriceInfo.class );

			if (0 < stockPriceInfo.getStockPrices().size()) {
