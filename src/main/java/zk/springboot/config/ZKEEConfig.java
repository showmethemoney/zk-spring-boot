...
public class ZKEEConfig {
    ...
    public ServletContextInitializer manualServletConfigInit() {
        return servletContext -> {
            ...
            servletContext.setAttribute("org.zkoss.zkmax.ws.filter.installed", true); // when FR ZK-3799 is
            // ready (8.5.1 ?)
        };
    }
    ...
    public FilterRegistrationBean wsFilter() {
        ...
        // reg.addUrlPatterns(WebSocketWebAppInit.getWebSocketUrl() + "/*"); // when FR ZK-3799 is ready
        // (8.5.1 ?)
        return reg;
    }
}
